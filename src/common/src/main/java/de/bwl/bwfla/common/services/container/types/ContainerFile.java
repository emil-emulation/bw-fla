...
public class ContainerFile extends File
{
	...
	/** Logger instance. */
	private final Logger log = LoggerFactory.getLogger(ContainerFile.class);
	...
	public boolean delete()
	{
		...
		if (refsleft > 0) {
			String message = "The file '{}' has still {} reference(s) after timeout! Deleting it will probably fail.";
			log.warn(message, this.getAbsolutePath(), refsleft);
		}
		
		final boolean success = super.delete();
		if (!success)
			log.warn("'{}' could not be deleted!", this.getAbsolutePath());
