...
public class Fits 
{
  private static Logger logger = Logger.getLogger(Fits.class.getSimpleName());
  ...
  {   
	...
	if(FITS_HOME == null)
	{
		String propFile = "/" + this.getClass().getSimpleName().toLowerCase() + ".properties";
		InputStream resource = this.getClass().getResourceAsStream(propFile);			
		
		if(resource == null)
			throw new FitsConfigurationException("neither FITS_HOME specified as a constructor parameter nor FITS property file not found inside the module under: " + propFile);
		
		try 
		{
			Properties properties = new Properties();
			properties.load(resource);
			FITS_HOME = properties.getProperty("fits.home");
			
			if(FITS_HOME == null)
				throw new FitsConfigurationException("'FITS_HOME' cannot be set, create a 'fits.home' property in the property file or specify home location as a 'Fits' constructor parameter");
		} 
		catch (IOException e) 
		{
			if(FITS_HOME == null)
				throw new FitsConfigurationException("fits property file could not be loaded from the module under: " + propFile);
		}
	}
	
	if(FITS_HOME.isEmpty() || !(new File(FITS_HOME)).isDirectory())
		throw new FitsConfigurationException("'FITS_HOME' directory does not exist on the filesystem under the following path: " + FITS_HOME);
