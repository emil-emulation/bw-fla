...
# Start websockify
#pwd=`pwd`
#len=`expr match "$pwd" '/.*/codebase'`
#websockify_dir=$(echo $pwd | awk -v var=$len '{ string=substr($0, 1, var); print string; }' )'/suite/resources/websockify/'
#websockify_bin=$websockify_dir'websockify'
#websockify_cert=$websockify_dir'bwfla-5840987166956866.pem'
#websockify_key=$websockify_dir'bwfla.key'
#$websockify_bin --target-config=/dev/null 6900 --cert=$websockify_cert --key=$websockify_key &
#websockify_pid=$!
...
while true; do
   if [ "x$LAUNCH_JBOSS_IN_BACKGROUND" = "x" ]; then
      ...
      trap "kill -HUP  $JBOSS_PID && kill $websockify_pid" HUP
      trap "kill -TERM $JBOSS_PID && kill $websockify_pid" INT
      trap "kill -QUIT $JBOSS_PID && kill $websockify_pid" QUIT
      trap "kill -PIPE $JBOSS_PID && kill $websockify_pid" PIPE
      trap "kill -TERM $JBOSS_PID && kill $websockify_pid" TERM
